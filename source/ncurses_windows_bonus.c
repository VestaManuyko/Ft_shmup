...
{
	wbkgd(visual->world.win, COLOR_PAIR(COLOR_WHITE_ON_DARK_BLACK));
	wbkgd(visual->infobox.win, COLOR_PAIR(COLOR_INFOBOX_BOX));
	box(visual->infobox.win, 0, 0); // draw border for infobox
	wrefresh(visual->world.win);
	wrefresh(visual->infobox.win);
	wbkgd(visual->infobox.win, COLOR_PAIR(COLOR_WHITE_ON_DARK_BLACK));
}
